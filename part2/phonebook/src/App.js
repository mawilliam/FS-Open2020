      ...
      personService
        ...
        .then((returnedPerson) => {
          ...
          if (returnedPerson) {
            ...
            setNewName('');
            setNewNumber('');
            setFilteredPersons(filteredPersons.map((person) => (
              person.id !== duplicate.id ? person : returnedPerson
            )));
            ...
            setErrorType('error');
            setErrorMessage(`${duplicate.name} was already removed from the server`);
            setTimeout(() => {
              setErrorMessage(null);
            }, 3000); // leave the message for at least 3 seconds
          }
  ...
  const deletePersonOf = (personID) => {
    ...
    personService
      ...
      .then(() => {
        ...
        setErrorType('message');
        setErrorMessage(`${toDelete.name} was removed from the phonebook`);
        setTimeout(() => {
          setErrorMessage(null);
        }, 3000); // leave the message for at least 3 seconds
      })
      .catch((error) => {
        setErrorType('error');
        setErrorMessage(`${toDelete.name} was already removed from the server`);
        setTimeout(() => {
          setErrorMessage(null);
        }, 3000); // leave the message for at least 3 seconds
      });
